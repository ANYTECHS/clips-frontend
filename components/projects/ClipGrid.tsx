        ...
        >
          {virtualizer.getVirtualItems().map((virtualRow: any) => {
            ...
            return (
              ...
              >
                <div data-tour="clip-grid" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 h-full">
                  {chunk.map((clip: any) => {
                    const isSelected = selectedIds.includes(clip.id);
                    const isRecommended = aiRecommendations && clip.score >= recommendationThreshold;

                    return (
                      <div key={clip.id} data-tour="clip-preview" className={`group relative rounded-2xl overflow-hidden contain-layout-style transition-all duration-300 border-2 ${isSelected ? "border-brand shadow-[0_0_20px_rgba(var(--brand),0.3)]" : "border-transparent hover:border-white/20"}`}>
