...
import { BarChart3,Check, Edit2, Play, Sparkles } from "lucide-react";
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={`skeleton-${i}`}
            className="animate-pulse bg-white/5 rounded-2xl aspect-[9/16] w-full"
          />
        ))}
      </div>
    );
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white/5 p-4 rounded-2xl">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${aiRecommendations ? "bg-brand/20 text-brand" : "bg-white/5 text-muted-foreground hover:bg-white/10"}`}
          >
              ...
              >
                ...
                >
                  {chunk.map((clip: any) => {
                    ...
                    const isRecommended =
                      aiRecommendations && clip.score >= recommendationThreshold;

                    return (
                      <div
                        key={clip.id}
                        data-tour="clip-preview"
                        className={`group relative rounded-2xl overflow-hidden contain-layout-style transition-all duration-300 border-2 ${isSelected ? "border-brand shadow-[0_0_20px_rgba(var(--brand),0.3)]" : "border-transparent hover:border-white/20"}`}
                      >
                        <div
                        ...
                        >
                          ...
                          <div
                            className={`absolute top-3 right-3 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? "bg-brand border-brand" : "border-white/50 bg-black/50 group-hover:border-white"} ${isRecommended ? "top-10" : ""}`}
                          >
                            ...
                            {clip.tags && clip.tags.length > 0 && (
                              <div className="flex flex-wrap gap-1">
                                {clip.tags.slice(0, 2).map((tag: string) => (
                                  <span
                                    key={tag}
                                    className="inline-block px-2 py-0.5 bg-brand/20 text-brand text-xs rounded-md font-semibold border border-brand/30 truncate max-w-[calc(50%-2px)]"
                                  >
                                    {tag}
                                  </span>
                                ))}
                                {clip.tags.length > 2 && (
                                  <span className="inline-block px-2 py-0.5 bg-white/10 text-white/60 text-xs rounded-md font-semibold border border-white/20">
                                    +{clip.tags.length - 2}
                                  </span>
                                )}
                              </div>
