...
Collection routes accept `page` and `pageSize`; page size defaults to 20 and is capped at 100. The response `meta` contains the total, page count, and next/previous flags. `/api/explore/trending` keeps its cursor-based contract. Fixed catalogues are bounded and return the same metadata shape.
