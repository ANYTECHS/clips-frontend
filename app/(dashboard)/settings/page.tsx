 ...
 */
const SocialRecoveryConfig = dynamic(() => import("@/components/SocialRecoveryConfig"), {
  ssr: false,
  loading: () => (
...
export default function SettingsPage() {
  ...
  const handleNetworkChange = (network: "testnet" | "mainnet") => {
    ...
    showToast(`Wallet network set to ${network === "testnet" ? "Testnet" : "Mainnet"}`, "success");
  };
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Failed to import secret key. Make sure it starts with 'S' and is 56 characters.";
        ...
        {pageLoading ? (
          ...
          <>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {passkeyCompatibility?.isSafari && (
                        <p className="text-[11px] text-yellow-300 mt-2 max-w-md">
                          Safari works best with HTTPS and iCloud Keychain, Touch ID, Face ID, or a
                    ...
                    {passkeyCredentialId ? (
                      ...
                      <div className="space-y-3">
                        <p className="text-xs text-muted-foreground leading-relaxed">
                          Register a passkey to sign in with your fingerprint, face, or security key
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Enable advanced wallet tools to manage on-chain Stellar keys, import custom
                keypairs, and set up Social Recovery guardians.
              </p>
                      ...
                      <div className="space-y-5">
                        <div className="p-4 rounded-xl bg-black/40 border border-white/5">
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <span className="text-[10px] px-2 py-0.5 rounded-full bg-brand/10 border border-brand/25 text-brand uppercase font-mono">
                              Active
                            </span>
                          </div>
                        ...
                        <div className="p-4 rounded-xl bg-black/40 border border-white/5 flex flex-col md:flex-row justify-between md:items-center gap-4">
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground leading-normal max-w-md">
                              Never share your Stellar Secret Key. Export requires multiple
                              confirmations and supports encrypted download.
                            </p>
                            {showPrivateKey && (
                          ...
                          <div className="p-4 rounded-xl bg-black/40 border border-white/5 flex flex-col gap-4">
                            <div className="space-y-1">
                              ...
                              <p className="text-xs text-muted-foreground leading-normal max-w-2xl">
                                This 12-word phrase restores your wallet connection. Store it
                                offline. Words are hidden after 30 seconds and copied text is
                                cleared from the clipboard automatically.
                              </p>

                              <div
                                className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 mt-3 ${mnemonicRevealed ? "" : "blur-sm select-none"}`}
                              >
                                {stellarMnemonic
                                  .split(/\s+/)
                                  .filter(Boolean)
                            ...
                            <div className="flex flex-wrap gap-2 items-center">
                              ...
                              {mnemonicRevealed && (
                                ...
                                >
                                  <Copy className="w-3.5 h-3.5" />
                                  {mnemonicCopied
                                    ? `Copied (${mnemonicClipboardCountdown}s)`
                                    : "Copy all words"}
                                </button>
                              )}
                            </div>
                        ...
                        {/* Trustline Manager */}
                        <TrustlineManager publicKey={address!} secretKey={stellarSecret} />
                      </div>
                    ) : (
                      ...
                      <div className="space-y-5">
                        <div className="p-4 rounded-xl bg-white/[0.01] border border-white/5 border-dashed text-center">
                          ...
                          <p className="text-xs font-semibold text-muted-foreground">
                            Stellar Wallet is currently disconnected. Please connect or generate a
                            wallet to enable advanced modes.
                          </p>
                        </div>
                          ...
                          {importSuccess && (
                            <div className="p-3 bg-emerald-950/40 border border-emerald-500/20 rounded-xl text-center">
                              <span className="text-xs text-emerald-400 font-semibold">
                                Stellar private key imported successfully!
                              </span>
                            </div>
                          )}
        ...
        >
          <div className="w-full max-w-sm bg-[#080B0A] border border-white/10 rounded-2xl p-6 shadow-2xl relative">
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 id="export-modal-title" className="font-bold text-white text-sm">
                  Export Encrypted Backup
                </h2>
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Set a strong password to protect your secret key.
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleEncryptedExport} className="space-y-4">
              ...
              {exportError && <p className="text-xs text-red-400 font-semibold">{exportError}</p>}

              <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-3 text-[10px] text-yellow-300 leading-relaxed">
                <strong>Important:</strong> If you lose this password, your backup cannot be
                decrypted. Store both the file and password safely offline.
              </div>
