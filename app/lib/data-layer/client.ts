...
const SENSITIVE_PATH = /\/auth(?:\/|$)|\/api\/auth(?:\/|$)/i;
...
export function isNetworkError(error: unknown): boolean {
  ...
  if (error instanceof Error) {
    return /failed to fetch|networkerror|load failed|network_error/i.test(error.message);
  }
  return false;
}
...
function extractErrorMessage(data: unknown, fallback: string): string {
  if (data && typeof data === "object") {
    const record = data as { error?: unknown; message?: unknown; errors?: Array<{ message?: string }> };
