 ...
 * matching dedup window/in-flight entries so invalidation cannot serve a
 * just-invalidated in-flight result.
 */
...
function mirrorRequestCacheInvalidation(keys: string[], tag?: string): void {
  if (tag) {
...
export function invalidateKeys(keys: string[]): void {
  ...
  if (unique.length) {
    mirrorRequestCacheInvalidation(unique);
    notify(unique);
  }
}
...
export function invalidatePrefix(prefix: string): void {
  ...
  requestCache.invalidateWhere((key) => key.startsWith(prefix) || key.includes(prefix));
